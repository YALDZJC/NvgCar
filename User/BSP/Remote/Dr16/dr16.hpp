...
class Dr16
{
  public:
    ...
    // 解析数据
    void Parse(HAL::UART::IUartDevice *uart_dev, int Size);
    bool ISDir();
  ...
  private: // 私有成员函数
    ...
    void UpdateStatus();
    // 清除ORE错误
    void ClearORE(HAL::UART::IUartDevice *uart_dev, uint8_t *pData, int Size);
  ...
  private:
    ...
    BSP::WATCH_STATE::StateWatch state_watch_{100}; // 100ms超时，可根据实际调整
